...
***17.3% of applicable respondents have been harrassed by their superiors or co-workers.***
...
ggplot(gss16, aes(x = email)) +
  geom_histogram(binwidth = 30, alpha = 1) +
  labs(title = "Distribution of Time Spent on Email Weekly",
       x = "Minutes spent on email weekly",
       y = "Frequency")
```

```{r}
mean_time <- mean(gss16$email, na.rm = TRUE)
median_time <- median(gss16$email, na.rm = TRUE)

mean_time
median_time
```

***The median is a better measure of the typical amount of time Americans spend on email weekly because it is less likely to be skewed by outliers.***
...
``` {r}
total_respondents <- gss16 %>%
  filter(!is.na(snap_insta)) %>%  
  summarise(count = n())

yes_respondents <- gss16 %>%
  filter(snap_insta == "Yes") %>%  
  summarise(count = n())

percentage_yes <- (yes_respondents$count / total_respondents$count) * 100

percentage_yes
```

***The percentages of `Yes's` is 37.46356.***
...
$$\widehat{email}_i = 120.5 - 3.2\times wrkstatPart-time_i + 30.5\times wrkstatFull-time + 25.3×snap-instaYes_i$$
...
```{r}
gss16_predictions <- augment(model$fit)
```

```{r}

ggplot(gss16_predictions, mapping = aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline (yintercept = 0, color = "gray", lty = "dashed") +
  labs(x = "Predicted Values", y = "Residuals", title = "Predicted Values vs Residuals")
```

***I do not see any major issues with the model.***
